...
<div class="min-h-screen bg-white text-black font-sans selection:bg-purple-500/30">
  ...
  <section class="relative pt-32 pb-20 md:pt-48 md:pb-32 px-6 overflow-hidden">
    ...
    <div class="relative max-w-4xl mx-auto text-center z-10">
      <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-gray-100 border border-gray-300 mb-8 backdrop-blur-sm">
        <span class="w-2 h-2 rounded-full bg-purple-500 animate-pulse"></span>
        <span class="text-xs font-medium text-purple-700">AI-Powered Study Platform</span>
      </div>
      ...
      <form class="flex flex-col space-y-4 w-full max-w-md mx-auto" on:submit|preventDefault={handleSubmit}>
        <textarea
          ...
          class="w-full p-4 bg-gray-50 border border-gray-300 rounded-lg text-black placeholder-gray-500 focus:border-purple-500 focus:outline-none resize-none"
          rows="4"
          bind:value={prompt}
        ></textarea>
        <input
          type="file"
          multiple
          class="w-full p-4 bg-gray-50 border border-gray-300 rounded-lg text-black file:bg-purple-600 file:text-white file:border-none file:rounded file:px-3 file:py-1 file:mr-3 hover:file:bg-purple-700"
          bind:files={files}
        />
        <button
          type="submit"
  ...
  <section id="studies" class="py-24 px-6 bg-white">
  ...
  <footer class="py-8 border-t border-gray-200 text-center text-gray-400 text-sm">
    <div class="flex items-center justify-center gap-2 mb-4">
      <div class="p-1 bg-gradient-to-tr from-purple-600 to-pink-600 rounded-lg">
        <Brain size={14} class="text-white" />
      </div>
      <span class="text-gray-800 font-bold">5bytes</span>
    </div>
